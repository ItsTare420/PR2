...
string GenerisiSifru(string imePrezime, int broj) {
	string sifra;
	time_t godina = time(0);
	tm trenutnoVrijeme{};
	localtime_s(&trenutnoVrijeme, &godina);
	int cifreGodine = trenutnoVrijeme.tm_year % 100;
	sifra += to_string(cifreGodine);
...
bool ValidirajSifru(string sifra) {
	return 
		regex_match(sifra, regex("[0-9]{2}-[A-Z]{2}[-\\s][0-9]{3}"));
}
...
public:
	...
	KolekcijaG1* RemoveRange(int start, int brojElemenata) {
		int kraj = start + brojElemenata - 1;
		if (start<0 || kraj >= getTrenutno() || start>kraj)
			throw exception("Opseg za uklanjanje elemenata nije validan!\n");

		KolekcijaG1* novaKolekcija = new KolekcijaG1;
		...
		for (int i = start; i < _trenutno-brojElemenata; i++) {
			_elementi1[i] = _elementi1[i + brojElemenata];
			_elementi2[i] = _elementi2[i + brojElemenata];
		}
...
public:
	Predavanje(Termin& t1, const char* tema, Specializacija spec):Dogadjaj(t1) {
	...
	string Info()const {
		ostringstream info{};
		info << getTermin() << " " << GetTema() << " " << GetSpecijalizacija() << endl;
		return info.str();
	}
...
public:
	Ucesnik(const char* imePrezime) {
		...
		_sifra = AlocirajNiz(GenerisiSifru(_imePrezime, _id++).c_str());
	}
	~Ucesnik() {
		delete[] _imePrezime;
		delete[] _sifra;
	...
	bool PrijaviSe(const Dogadjaj& dog) {
		for (int i = 0; i < _prijavljeni.size(); i++) {
			if (_prijavljeni[i]->Info() == dog.Info())
				return false;
		}
		_prijavljeni.push_back(dog.clone());
		return true;
	}
...
public:
	...
	void DodajUcesnika(Ucesnik& obj) {
		for (auto& ucesnik : GetClanovi()) {
			if (strcmp(ucesnik.getSifra(), obj.getSifra()) == 0)
				throw exception("Proslijedjeni ucesnik je vec dio tima!\n");
		}
		_clanovi.push_back(obj);
	}
	friend ostream& operator <<(ostream& os, const Tim& obj) {
		os << "--- Tim: " << obj.getNaziv()<<" ---" << endl;
		for (auto& ucesnik : obj._clanovi) {
			os << "\t" << ucesnik.getImePrezime() << " (" << ucesnik.getSifra() << ")" << endl;
		}
		return os;
	}
};
...
public:
	...
	bool PrijaviDogadjaj(const char* nazivTima,const char* sifra,const Dogadjaj& dogadjaj) {
		Tim* timZaPronaci = nullptr;
		for (int i = 0; i < _timovi.getTrenutno(); i++) {
			if (strcmp(_timovi.getElement1(i)->getNaziv(), nazivTima) == 0) {
				timZaPronaci = _timovi.getElement1(i);
				break;
			}
			if (strcmp(_timovi.getElement2(i)->getNaziv(), nazivTima) == 0) {
				timZaPronaci = _timovi.getElement2(i);
				break;
			}
		}
		if (timZaPronaci == nullptr)
			return false;
		for (auto& ucesnik : timZaPronaci->GetClanovi()) {
			if (strcmp(ucesnik.getSifra(), sifra) == 0) {
				return ucesnik.PrijaviSe(dogadjaj);
			}
		}
		return false;
	}

	friend ostream& operator<<(ostream& os, const Konferencija& obj) {
		os << "\n======= Konferencija: " << obj.getNaziv()<<" =======" << endl;
		for (int i = 0; i < obj._timovi.getTrenutno(); i++) {
			os << *obj._timovi.getElement1(i);
			os << *obj._timovi.getElement2(i);
		}
		return os;
	}

	vector<Ucesnik*> operator ()(int brojPredavanja) {
		vector<Ucesnik*> redovniUcesnici;
		for (int i = 0; i < _timovi.getTrenutno(); i++) {
			for (auto& ucesnik : _timovi.getElement1(i)->GetClanovi()) {
				if (ucesnik.getDogadjaji().size() >= brojPredavanja) {
						redovniUcesnici.push_back(&ucesnik);
				}
			}
			for (auto& ucesnik : _timovi.getElement2(i)->GetClanovi()) {
				if (ucesnik.getDogadjaji().size() >= brojPredavanja) {
						redovniUcesnici.push_back(&ucesnik);
				}
			}
		}
		return redovniUcesnici;
	}

};
bool UcitajUcesnike(const char* fajl, vector<Tim>& timovi) {
	ifstream ulaz(fajl);
	if (!ulaz.is_open()) return false; 
	string linija;
	while (getline(ulaz, linija)) {
		...
		string nazivTima = linija.substr(pozicija + 1);
		Tim* ciljniTim = nullptr;
		for (auto& tim : timovi) {
			if (strcmp(tim.getNaziv(), nazivTima.c_str()) == 0) {
				ciljniTim = &tim;
				break;
			}
		}
		if (ciljniTim == nullptr) {
			timovi.push_back(Tim(nazivTima.c_str()));
			ciljniTim = &timovi.back();
		}
		bool pronadjen = false;
		for (auto& ucesnik : ciljniTim->GetClanovi()) {
			if (strcmp(ucesnik.getImePrezime(), imePrezime.c_str()) == 0) {
				pronadjen = true;
				break;
			}
		}
		if (!pronadjen) {
			...
			try {
				ciljniTim->DodajUcesnika(noviUcesnik);
			}
			catch (...) {
			}
		}
