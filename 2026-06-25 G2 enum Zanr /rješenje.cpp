...
public:
	...
	Kolekcija(const Kolekcija& obj)
	{
		...
		_prvi = new T1[obj._trenutno];
		_drugi = new T2[obj._trenutno];
	...
	void Dodaj(const T1& el1, const T2& el2) {
		T1* const noviPrvi=new T1[_trenutno + 1];
		T2* const noviDrugi= new T2[_trenutno + 1];

		for (int i = 0; i < _trenutno; i++) {
			noviPrvi[i] = GetPrvi(i);
			noviDrugi[i] = GetDrugi(i);
		}

		noviPrvi[_trenutno] = el1;
		noviDrugi[_trenutno] = el2;
	...
	Kolekcija& DodajNaPoziciju(int lokacija, const T1& el1, const T2& el2) {
		...
		T1* noviPrvi = new T1[_trenutno + 1];
		T2* noviDrugi = new T2[_trenutno + 1];
		int brojacStarih = 0; 
		for (int i = 0; i < _trenutno + 1; i++) {
			if (i == lokacija) {
				...
				noviPrvi[i] = _prvi[brojacStarih];
				noviDrugi[i] = _drugi[brojacStarih];
				brojacStarih++;
			}
		}
		delete[] _prvi;
	...
	Kolekcija* UkloniRaspon(int start, int brojElemenata) {
		...
		if (start < 0 || brojElemenata <= 0 || end >= _trenutno)
			throw exception("Opseg za uklanjanje elemenata nije validan!");

		Kolekcija* uklonjeniElementi = new Kolekcija();
		for (int i = start; i <= end; i++) {
			uklonjeniElementi->Dodaj(_prvi[i], _drugi[i]);
		}
		int noviKapacitet = _trenutno - brojElemenata;
		T1* noviPrvi = nullptr;
		T2* noviDrugi = nullptr;

		if (noviKapacitet > 0) {
			noviPrvi = new T1[noviKapacitet];
			noviDrugi = new T2[noviKapacitet];

			int j = 0;
			for (int i = 0; i < _trenutno; i++) {
				if (i < start || i > end) {
					noviPrvi[j] = _prvi[i];
					noviDrugi[j] = _drugi[i];
					j++;
				}
			}
		}
...
public:
	...
	string ToString()const {
		stringstream info;
		info << _datumPosudbe.ToString() << " " << _naslov<< " " << ZanrNazivi[_zanr] << " " << _brojDana << " dana";
			return info.str();
	}
...
public:
	...
	bool RazduziKnjigu(const char* naslovKnjige) {
		for (auto i = _posudbe.begin(); i != _posudbe.end();i++) {
			if (strcmp(i->GetNaslov(), naslovKnjige) == 0) {
				_posudbe.erase(i);
				return true;
			}
		}
...
public:
	...
	void DodajClana(const ClanBiblioteke& noviClan) {
		for (auto& clan : _clanovi) {
			if (strcmp(clan.GetClanskiBroj(), noviClan.GetClanskiBroj()) == 0)
				throw exception("Dodavanje duplih clanova nije moguce!");
		}
	...
	bool EvidentirajPosudbu(const char* clanskiBroj, const Posudba& novaPosudba) {
		for (auto& clan : _clanovi) {
			if (strcmp(clan.GetClanskiBroj(), clanskiBroj) == 0)
				return clan.DodajPosudbu(novaPosudba);
		}
		return false;
	}
	...
	Kolekcija<ClanBiblioteke, int, 50> PosudbePoZanru(Zanr zanr) {
		Kolekcija<ClanBiblioteke, int, 50> rezultat;
		for (auto& clan : _clanovi) {
			for (auto& posudba : clan.GetPosudbe()) {
				if (posudba.GetZanr() == zanr)
					rezultat.Dodaj(clan, clan.GetPosudbe().size());
			}
		}
...
bool UcitajPodatke(const char* nazivFajla, vector<Biblioteka>& biblioteke) {
	...
	while (getline(fajl, linija)) {
		...
		string nazivBiblioteke = linija.substr(0, pozicija);
		string imeClana = linija.substr(pozicija + 1);
		if (!imeClana.empty() && imeClana.back() == '\r') {
			imeClana.pop_back();
		}
		Biblioteka* trazenaBiblioteka = nullptr;
		for (auto& biblioteka : biblioteke) {
			if (strcmp(biblioteka.GetNaziv(), nazivBiblioteke.c_str()) == 0) {
				trazenaBiblioteka = &biblioteka;
				break;
			}
		}
		if (trazenaBiblioteka == nullptr) {
			biblioteke.push_back(Biblioteka(nazivBiblioteke.c_str()));
			trazenaBiblioteka = &biblioteke.back();
			ucitanoNestoNovo = true;
		}
		bool clanPostoji = false;
		for (auto& clan : trazenaBiblioteka->GetClanovi()) {
			if (strcmp(clan.GetImePrezime(), imeClana.c_str()) == 0) {
				clanPostoji = true;
				break;
			}
		}
		if (!clanPostoji) {
			trazenaBiblioteka->DodajClana(ClanBiblioteke(imeClana.c_str()));
			ucitanoNestoNovo = true;
		}
	}
	fajl.close();
...
const char* GetOdgovorNaPrvoPitanje() {
	cout << "Pitanje -> Pojasnite kako tip nasljeđivanja (public,protected, private) utice na dostupnost clanova bazne klase u" 
		"izvedenoj klasi.Sta se desava sa public i protected clanovima" 
		"bazne klase ako se nasljeđivanje izvrsi kao protected ili private ? .\n";
		return "Odgovor -> OVDJE UNESITE VAS ODGOVOR";
}
const char* GetOdgovorNaDrugoPitanje() {
	cout << "Pitanje -> Pojasnite ulogu i znacaj move konstruktora,"
		"kada se poziva i po cemu se razlikuje od konstruktora kopije.\n";
		return "Odgovor -> OVDJE UNESITE VAS ODGOVOR";
}
int main() {
	...
	// (za potrebe testiranja mozete dodati try catch blok).
	...
	try {
		//DodajClana onemogucava dodavanje clana sa istim clanskim brojem i baca izuzetakFakultet informacijskih tehnologija
		gradska.DodajClana(amina);
	}
	catch (exception& e) {
		cout << "Exception: " << e.what() << crt;
